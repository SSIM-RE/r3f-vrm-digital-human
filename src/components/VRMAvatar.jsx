...
export const VRMAvatar = ({ avatar, animation, ...props }) => {
  ...
  const {
    ...
    lookAtMouse: { value: false, label: "注视鼠标" },
  });
  ...
  for (const key in expressions) {
    if (emotionKeys.includes(key)) {
      limitedExpressions[key] = Math.min(expressions[key], maxEmotionScale);
    } else {
  ...
  for (const key in targetExpressions) {
    if (emotionKeys.includes(key)) {
      limitedTarget[key] = Math.min(targetExpressions[key], maxEmotionScale);
    } else {
